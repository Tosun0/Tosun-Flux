...
설치파일은 `make-installer.ps1`로 생성합니다. 설치 프로그램에서 설치 폴더와 바탕화면·시작 메뉴 바로가기를 선택할 수 있으며, 기본 위치는 `C:\Program Files\Tosun Flux`입니다. Windows의 프로그램 설치 및 제거 목록과 App Paths에 게시자 `Tosun`, 버전 `0.4.0`으로 등록됩니다. 설치 완료 전 포함된 변환 백엔드 상태를 검사하며, 설치 시 관리자 권한이 필요합니다.
